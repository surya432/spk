...
trait HelperController
{
    // x3 = umur, x2 = gaji, x1 = nilai jaminan, x4 = nilai milik sendiri 
     function variabelX3($umur, $batasUmur)
    {
        return $umur <= $batasUmur ? 1 : 0;
    }

     function variabelX4($pengajuan, $sisaAsset)
    {
        return $pengajuan >= $sisaAsset ? 1 : 0;
    }
     function variabelX2($nilaiPengajuan, $bunggaKPR, $tenor, $batasMinimTungakkan, $gajiBersih)
    {

        $dataA = ($nilaiPengajuan * $bunggaKPR / 100) + ($nilaiPengajuan /  $tenor) * $batasMinimTungakkan;
        $dataB = $gajiBersih / $batasMinimTungakkan;
        return $dataA > $dataB ? 1 : 0;
    }

     function variabelX1($nilaiPengajuan, $nilaiJaminan)
    {
        $dataA =  $nilaiJaminan / 2;

        if ($nilaiPengajuan < $dataA) {
            $nilaiCoverages = $this->nilaiCoverage($nilaiJaminan, $nilaiPengajuan);
     ...
     function nilaiCoverage($nilaiJaminan, $nilaiPengajuan)
    {
        $dataA =  $nilaiPengajuan / $nilaiJaminan * 100;
        return $dataA;
    }
     function convertTo($nilaiJaminan, $nilaiPengajuan)
    {
        $dataA =  $nilaiPengajuan / $nilaiJaminan * 100;
        return $dataA;
    }
    ...
    public function hitung($kdPengajuan)
    {
        ...
        $valBias = \App\DataTraining::where('keys', 'bias')->first();
        $bias = $valBias['value'];
        $activasi = 0;
    ...
    public function hitungOutput($kdPengajuan, $bias)
    {
        ...
        for ($j = 1; $j <= $hiddenLayer->value; $j++) {
            $xw = 0;
            $a = \App\DataTraining::where(['keys' => 'z' . $j, 'pengajuan_id' => $kdPengajuan])->first();
    ...
    public function errorY($kdPengajuan, $bias)
    {
        ...
        for ($j = 1; $j <= $hiddenLayer->value; $j++) {
            ...
            $inputLayer = \App\DataTraining::where(['keys' => 'z' . $j, 'pengajuan_id' => $kdPengajuan])->first();
            //E3 * (B18 - E3) * (E14 - B14)
    ...
    public function BobotBaruLayer($kdPengajuan, $bias)
    {
        ...
        for ($j = 1; $j <= $hiddenLayer->value; $j++) {
            ...
            $hasilErrorOutput = $this->numberlimit($hitungOutput);
            \App\DataTraining::create(['keys' => 'z' . $j, 'value' => $hasilErrorOutput]);
        }
